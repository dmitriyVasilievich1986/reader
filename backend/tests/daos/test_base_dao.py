...
Uses ``AuthorDAO`` as the concrete subject because ``BaseDAO`` itself is
abstract; the behaviour exercised here lives entirely in ``BaseDAO``.
"""
...
class TestInit:
    ...
    def test_stores_database_client_when_provided(
        self, db_client: AsyncDatabaseClient, migrated_db: None,
    ) -> None:
...
class TestCreate:
    ...
    async def test_create_uses_db_client_path(
        self, migrated_db: None, db_client: AsyncDatabaseClient,
    ) -> None:
        """When no session is injected the DAO opens a session from the client."""
        dao = AuthorDAO(database_client=db_client)
